...
# ===================== 核心路径配置 =====================
source_img_dir = r"D:\sy\crack"  # 原始图片目录（100张：前50石、后50木）
source_label_dir = r"D:\sy\标签\crack"  # 原始标签目录
train_root = r"D:\sy\crack_dataset\train"  # 训练集根目录
val_root = r"D:\sy\验证集\crack"  # 验证集根目录（指定路径）
val_ratio = 0.2  # 验证集比例（20%）
random_seed = 42  # 固定随机种子，结果可复现
# ==================================================

# 1. 创建目录结构（训练集：images/labels；验证集：images/labels）
...
# 4. 复制验证集图片+标签到指定路径（D:\sy\验证集\crack）
...
# 5. 生成最终的YOLO配置文件（指向指定的训练/验证集路径）
yaml_content = f"""
# 石制/木制裂缝数据集配置（验证集路径：D:\sy\验证集\crack）
train: {train_img_dir}  # 训练集图片路径
val: {val_img_dir}      # 验证集图片路径（指定到D:\sy\验证集\crack\images）
nc: 2                   # 类别数：石制裂缝+木制裂缝
names: ["stone_crack", "wood_crack"]  # 类别名对应ID 0/1
"""
# 配置文件保存到训练集根目录，方便调用
yaml_path = os.path.join(train_root, "../crack_dataset.yaml")
